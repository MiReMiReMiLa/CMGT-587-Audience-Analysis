...
OUTPUT = "../outputs/scored_reviews_part2.csv"

df = pd.read_csv(INPUT)

# ---------- START INDEX ----------
start_idx = 3237  # start from row 3238 (0-based)
...
for i, text in enumerate(
    tqdm(df["text"].iloc[start_idx:], total=len(df) - start_idx)
):

    result = score_one(text)

    # ✅ safe fallback (avoid None breaking your dataset)
    if result is None:
        result = {
            "cleanliness": None,
            "amenities": None,
            "winter_amenities": None,
            "view": None,
            "coziness": None,
            "activities": None,
            "luxury": None,
